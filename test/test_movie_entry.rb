...
class TestMovieEntries < MovieTest
  ...
  def test_all_movie_entries_in_alphabetical_order
    database.execute("insert into movie_entries(title, seen, own, wishlist_see, wishlist_own, user_rating) values('foo', 't', 'f', 't', 't', 42)")
    database.execute("insert into movie_entries(title, seen, own, wishlist_see, wishlist_own, user_rating) values('bar', 'f', 'f', 'f', 't', 67)")
